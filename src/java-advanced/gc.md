...
:::tip GC 收集器选择决策树
1. **堆 < 4GB** → Parallel GC（默认，吞吐量优先）
2. **4GB < 堆 < 32GB，关注暂停** → G1
3. **堆 > 32GB 或极低延迟要求** → ZGC（JDK 21+ 分代 ZGC）
4. **已废弃** → CMS（JDK 14 移除）、Serial（仅限单核/嵌入式）
:::
|--------|------|------|------|------|--------|---------|
...
:::tip G1 调优建议
1. **不要手动设置年轻代大小**（-Xmn）：G1 会自动调整，手动设置会破坏 G1 的启发式算法
2. **核心参数**：`-XX:MaxGCPauseMillis=200`（目标暂停时间，默认 200ms）
3. **大堆推荐**：堆 > 6GB 时优先选 G1，> 32GB 考虑 ZGC
4. **监控指标**：Mixed GC 频率、Humongous 对象数量、String 去重效果
:::
```bash
# ========== 核心参数 ==========
-XX:+UseG1GC                        # 启用 G1
...
:::info ZGC vs G1 如何选择？
- **ZGC**：暂停时间 < 1ms（JDK 16+），适合低延迟场景（交易系统、实时推荐）
- **G1**：暂停时间可控（100-200ms），适合通用场景，生态成熟
- **JDK 21+**：ZGC 支持分代架构，性能进一步提升，建议新项目直接上 ZGC
:::
