...
## ⭐ Redis 持久化机制：RDB 和 AOF 的区别？怎么选？

**简要回答：** RDB 是快照（二进制文件），AOF 是日志（追加写命令）。Redis 4.0+ 支持混合持久化（RDB + AOF 增量），推荐使用。

**深度分析：**

```bash
# RDB 配置
save 900 1        # 900秒内至少1个写操作则触发
save 300 10       # 300秒内至少10个写操作
save 60 10000     # 60秒内至少10000个写操作
save ""          # 禁用自动 RDB（使用 save ""）

# AOF 配置
appendonly yes
appendfsync everysec  # 每秒刷盘（推荐，最多丢1秒数据）
# always: 每次写操作都刷盘（最安全，性能最差）
# no: 由操作系统决定（最快，最多丢操作系统缓冲区数据）

# 混合持久化（Redis 4.0+）
aof-use-rdb-preamble yes  # AOF 文件开头用 RDB 格式，后续增量用 AOF
```
...
| 恢复速度 | 快 | 慢（逐条重放） | 快（RDB 加载 + AOF 增量重放） |
| 性能影响 | fork 时可能有瞬间卡顿 | 每秒 fsync | fork 时有瞬间卡顿 |
| 适用场景 | 备份、灾难恢复 | 数据安全要求高 | **推荐方案** |

:::tip 面试追问
- **fork 导致的阻塞怎么解决？** 控制 Redis 实例大小（建议 < 10GB）、使用 `auto-aof-rewrite-min-size` 控制 AOF 重写频率
- **AOF 重写是什么？** 将多条写命令合并为少量命令，减小 AOF 文件体积（`bgrewriteaof`）
- **Redis 4.0 之前的选型？** 如果能容忍少量丢失用 RDB（性能好），不能容忍用 AOF
:::
...
**简要回答：** 聚簇索引（InnoDB 主键索引）的叶子节点存完整数据行，非聚簇索引（二级索引）的叶子节点存主键值。通过二级索引查数据需要回表（回聚簇索引查完整行）。
...
-- 1. 先在 idx_name 索引树找到主键 id
-- 2. 再用 id 回聚簇索引树查完整行（回表）

-- 覆盖索引（不需要回表）
SELECT id, name FROM user WHERE name = '张三';
-- idx_name 的叶子节点已经有 id 和 name，无需回表
```
...
完整数据行]
    ...
    style N1 fill:#fef9e7,stroke:#f39c12
    style P1 fill:#eafaf1,stroke:#27ae60
```

| 对比 | 聚簇索引 | 非聚簇索引（二级索引） |
|------|---------|------------------|
| 叶子节点存储 | 完整数据行 | 主键值 + 索引列 |
| 数量 | 一张表只有一个 | 可以有多个 |
| 回表 | 不需要（数据就在叶子节点） | 可能需要（查非索引列时） |
| 排序 | 物理上按主键排序 | 按索引列排序 |

:::tip 面试追问
- **MyISAM 有聚簇索引吗？** 没有，MyISAM 的所有索引都是非聚簇索引（叶子节点存数据行指针）
- **为什么 InnoDB 建议用自增主键？** 自增主键保证数据按顺序插入，页分裂少，聚簇索引更紧凑
- **什么是索引下推（ICP）？** MySQL 5.6+ 优化，在索引遍历过程中对 WHERE 条件过滤，减少回表次数
:::
